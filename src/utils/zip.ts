// Minimal ZIP (store-only) writer for bundling files without compression
// Writes local headers, central directory, and EOCD. Uses current date/time zeros.

import { encode as enc } from './ziputil-enc';
...
export function makeZip(entries: ZipEntry[]): Uint8Array {
  ...
  for (const e of entries) {
    ...
    const ver = le16(20); // version needed
    const flags = le16(0);
    const comp = le16(0); // store
    const mtime = le16(0), mdate = le16(0);
    const crc = crc32(e.data);
    const csize = le32(e.data.length);
    const usize = le32(e.data.length);
    const nlen = le16(nameBytes.length);
    const elen = le16(0);
  ...
  for (const lh of localHeaders) {
    ...
    const verMade = le16(20), verNeed = le16(20);
    const flags = le16(0), comp = le16(0);
    const mtime = le16(0), mdate = le16(0);
    const crc = le32(lh.crc);
    const csize = le32(lh.data.length), usize = le32(lh.data.length);
    ...
    header.set(mtime, 12); header.set(mdate, 14); header.set(crc, 16);
    header.set(csize, 20); header.set(usize, 24);
    header.set(nlen, 28); header.set(elen, 30); header.set(clen, 32);
    header.set(disk, 34); header.set(iattr, 36); header.set(eattr, 38);
    header.set(off, 42);
  ...
  eocd.set(le16(0), 4); // disk
  eocd.set(le16(0), 6); // cd start disk
  eocd.set(le16(entries.length), 8); // num entries this disk
  eocd.set(le16(entries.length), 10); // total entries
  eocd.set(le32(cdSize), 12); // size of CD
  eocd.set(le32(cdOffset), 16); // offset of CD
  eocd.set(le16(0), 20); // comment len

  // Assemble final
  let total = 0;
  for (const lh of localHeaders) total += lh.header.length + lh.nameBytes.length + lh.data.length;
  for (const cp of centralParts) total += cp.length;
  total += eocd.length;
...
export function downloadZip(entries: ZipEntry[], filename = 'files.zip'){
